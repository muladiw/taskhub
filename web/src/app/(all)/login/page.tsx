...
import apiAuth, { LOGIN } from "@/api/authentication";
import { showToast } from "@/libs/helper";
import moment from "moment";
...
export default function Page() {
  ...
  const { mutateAsync: mutateLogin } = useMutation({
    ...
    onSuccess: (res: {
      ...
      if (res.isSuccess) {
        showToast("Berhasil", "Berhasil Login", "success");
        const waktuExp = moment().add(2, "M").toDate();

        queryClient.invalidateQueries();

        cookies.set("refreshToken", res.auth.refreshToken, {
          expires: waktuExp,
          path: "/",
        });
  ...
  const { mutateAsync: mutateAddUser } = useMutation({
    mutationFn: (data: LOGIN) => apiUser.postData(data),
    onSuccess: (res: {
      isSuccess: boolean;
      auth: { accessToken: string; refreshToken: string };
    }) => {
    ...
    <div className="h-screen">
      ...
      <footer className="absolute bottom-5">
        <span className="pl-10 font-light text-sm text-gray-700">
          muladiw.id©{new Date().getFullYear()}
        </span>
      </footer>
    </div>
  );
